...
def descr_to_dtype(descr):
    ...
    pp(fname + " name", name)
    pp(fname + " formats", formats)
    pp(fname + " titles", titles)
    pp(fname + " offsets", offsets)
    pp(fname + " offset", offset)
...
def _read_array_header(fp, version, max_header_size=_MAX_HEADER_SIZE):
    ...
    pp(fname + ' _hinfo', hinfo)

    if hinfo is None:
        raise ValueError("Invalid version {!r}".format(version))

    hlength_type, encoding = hinfo
    pp(fname + ' hlength_type', hlength_type)
    pp(fname + ' encoding', encoding)

    hlength_str = _read_bytes(fp, struct.calcsize(hlength_type), "array header length")
    pp(fname + ' hlength_str', hlength_str)

    header_length = struct.unpack(hlength_type, hlength_str)
    pp(fname + ' header_length', header_length)
    header_length = header_length[0]

    header = _read_bytes(fp, header_length, "array header")
    pp(fname + ' header', header)
...
def _read_bytes(fp, size, error_template="ran out of data"):
    ...
    while True:
        ...
        try:
            ...
            pp(fname, f' reading `r` {c} times', r)
            data += r
